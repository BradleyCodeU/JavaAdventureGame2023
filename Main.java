...
public class Main
{
 ...
 private static boolean isGameWon = false;
  ...
  public static void main(String[] args)
  {
    ...
    while(true)
    {
      ...
      else
      {
        ...
        typewriter(50, ". Valid options include: " + player.getCurrentRoom().getPossibleDirections() + (player.getCurrentRoom().getItem() == null ? "" : "take, ") + (player.getCurrentRoom().getCharacter() == null ? "" : "talk, fight, ") + "or quit.\n");
      }
  ...
  /*
    Pick up an item from the current room and put it in the player.getBackpack(). If something is in the player.getBackpack(), drop it in the room.
  */
  ...
  */
  public static int fight(Scanner input, Random rng, Player player)
  {
    ...
    {
      System.out.print("FIGHT!!! p = punch, k = kick, r = run" + (player.getBackpack() == null ? "" : ", x = use " + player.getBackpack().getName()) + ": ");
      String command = input.next();
      command = command.toLowerCase();
      if(command.equals("r"))
      {
  ...
  public static void enemyLoseHealth(Random rng, String command, Enemy e, Player player)
  {
    ...
    if(command.equals("p"))
    {
      ...
      attack = rng.nextInt(player.getPunchStrength()) + rng.nextInt(player.getPunchStrength()) + 1;
      if(attack >= 12)
      {
    ...
    else if(command.equals("k"))
    {
      ...
      attack = rng.nextInt(player.getKickStrength()) + rng.nextInt(player.getKickStrength()) + 1;
      if(attack >= 12)
      {
        typewriter(50, "It's super effective!\n");
      }
    }
    else if(command.equals("x") && player.getBackpack() != null)
    {
      typewriter(50, "You used " + player.getBackpack().getName().toUpperCase() + "\n");
      attack = rng.nextInt(player.getBackpack().getStrength()) + rng.nextInt(player.getBackpack().getStrength()) + rng.nextInt(player.getBackpack().getStrength()) + 1;
      if(player.getBackpack().getMagicType() == e.getMagicWeakness())
      {
        attack = attack * 3;
      }
      if(attack >= 12)
      ...
      player.getBackpack().weaken();
      if(player.getBackpack().isBroken())
      {
        typewriter(50, "Oh no! ! ! ! ! ! ! Your " + player.getBackpack().getName() + " broke! ! ! ! ! ! !\n");
        player.setBackpack(null);
      }
    }
    else if(!command.equals("r"))
    {
      ...
      typewriter(50, ". Valid options: p, k, r" + (player.getBackpack() == null ? "" : ", x = use " + player.getBackpack().getName()) + "\n");
      return;
    }
